...
/**
 * S3-compatible implementation of CVE service
 * Works with AWS S3, MinIO, DigitalOcean Spaces, etc.
 */
export class S3CVEService implements CVEServiceType {
   ...
   */
  private groupByContainer(objects: Array<{ key: string; size: number; lastModified: Date }>): Map<string, CVEfile[]> {
    const containerMap = new Map<string, CVEfile[]>();
    ...
    for (const obj of objects) {
      ...
      const file: CVEfile = {
        name: fileName,
        path: `/cves/${containerName}/${fileName}`,
        size: obj.size,
  ...
  /**
   * Lists CVE files with pagination and optional search
   */
  async listSboms(page: number = 1, search?: string): Promise<CVEListResponse> {
    try {
      ...
      console.error('Error listing CVEs from S3:', error);
  ...
  /**
   * Gets the S3 URL for a specific CVE file
   */
  ...
  /**
   * Returns all CVE JSON files for a specific container name
   */
  async listCVEFiles(containerName: string): Promise<CVEfile[]> {
  ...
  async saveFile(fileName: string, content: string): Promise<void> {
    // Deprecated – forward to saveCVE
    const [image, toolWithExt] = fileName.split('/');
    const [tool, ext] = toolWithExt.split('.');
    const type = ext?.toLowerCase() ?? '';
    return this.saveCVEFile(image, type, tool, content);
  }

  /**
   * Save an CVE for a given image, type, and tool.
   */
